...
export default function RiskPage() {
  return (
    <div className="space-y-8">
      ...
      {/* Risk Status Banner */}
      <div className="glass-card p-4 border-l-4 border-vault-accent">
        <div className="flex items-center gap-3">
          <div className="w-3 h-3 bg-vault-accent rounded-full animate-pulse" />
          <span className="font-medium">Risk Status: Normal</span>
          <span className="text-sm text-gray-400 ml-auto">All guardrails within limits</span>
        </div>
      </div>

      {/* Risk Gauges */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <RiskGauge label="Daily Drawdown" value={0} limit={3} unit="%" color="green" />
        <RiskGauge label="Monthly Drawdown" value={0} limit={8} unit="%" color="green" />
        <RiskGauge label="Net Delta" value={0} limit={10} unit="%" color="green" />
        <RiskGauge label="VaR (95%, 1D)" value={0} limit={2} unit="%" color="green" />
      </div>
  ...
  return (
    <div className="glass-card p-6">
      ...
      <div className="flex items-end gap-1 mt-2">
        ...
        <span className="text-gray-400 text-sm mb-1">/ {limit}{unit}</span>
      </div>
