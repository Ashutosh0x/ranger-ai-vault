// =================================================================
// Drift Executor -- Opens/closes perp positions on Drift protocol
...
// Drift market indexes for perps
...
export class DriftExecutor {
  ...
  async closePosition(asset: string): Promise<string | null> {
    ...
    const perpPosition = this.driftUser.getPerpPosition(marketIndex);
  ...
  async getPosition(asset: string): Promise<PositionInfo | null> {
    ...
    const unrealizedPnl =
      this.driftUser.getUnrealizedPNL(true, marketIndex).toNumber() /
    ...
    return {
      ...
      unrealizedPnl,
      fundingAccrued,
      leverage: this.driftUser.getLeverage().toNumber() / 10000,
    };
  }
