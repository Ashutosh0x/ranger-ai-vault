# 🏗️ Ranger AI Vault — System Architecture
...
graph TB
    ...
    subgraph Dashboard["🖥️ Dashboard (Next.js 14)"]
...
graph TB
    ...
    DE -->|"trade IX"| AA
    AA -->|"attested TX"| Solana["⛓️ Solana"]
    VA --> RE
    KL --> LG
    KL --> MT
    EU --> AL
...
```mermaid
graph TB
    subgraph OnChain["⛓️ On-Chain State"]
...
```mermaid
graph TB
    subgraph Failures["⚠️ Failure Modes"]
