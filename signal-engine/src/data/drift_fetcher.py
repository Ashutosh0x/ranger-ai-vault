...
Fetches funding rates, OI, volume, oracle prices from Drift API.
"""
...
class DriftFetcher:
    """Fetch market data from Drift Protocol's REST API."""

    def __init__(self):
        self.base_url = DRIFT_API_BASE
    ...
    def get_funding_rates(
        ...
        Drift API: /fundingRates?marketIndex=N
        APR = fundingRatePct * 24 * 365
        """
        market_info = DRIFT_MARKET_MAP.get(asset)
        if not market_info:
        ...
        try:
            ...
            print(f"[WARN] Drift funding rate fetch error for {asset}: {e}")
            return pd.DataFrame()
    ...
    def get_oracle_price(self, asset: str) -> Optional[float]:
        ...
        try:
            ...
            print(f"[WARN] Drift oracle price fetch error for {asset}: {e}")
        return None
    ...
    def get_market_data(self, asset: str) -> Dict:
        ...
        try:
            ...
            print(f"[WARN] Drift market data fetch error for {asset}: {e}")
        return {}
    ...
    def get_ohlcv(
        ...
        try:
            ...
            print(f"[WARN] Drift OHLCV fetch error for {asset}: {e}")
            return pd.DataFrame()
    ...
    def get_trades(self, asset: str, limit: int = 100) -> pd.DataFrame:
        ...
        try:
            ...
            print(f"[WARN] Drift trades fetch error for {asset}: {e}")
            return pd.DataFrame()
...
# Singleton
_fetcher: Optional[DriftFetcher] = None


def get_drift_fetcher() -> DriftFetcher:
    global _fetcher
    if _fetcher is None:
        _fetcher = DriftFetcher()
    return _fetcher
